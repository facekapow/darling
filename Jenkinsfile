pipeline {
	...
	environment {
		...
		REAL_WORKSPACE = "/home/jenkins/workspace/darling/${JOB_NAME}"
	}
	...
	stages {
		...
		stage('Build') {
			steps {
				dir("${REAL_WORKSPACE}") {
					dir('source') {
						// hack that's only necessary for the testing grounds
						...
						//sh 'tools/makedeb --dsc'
						sh 'tools/makedeb'

						// again, hack
						sh 'git remote set-url origin https://github.com/facekapow/darling.git'
					}
